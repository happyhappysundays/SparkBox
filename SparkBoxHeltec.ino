...
#define VERSION "BLE 0.49"
#define MAXNAME 20

SparkIO spark_io(true);               // Non-passthrough Spark IO (per Paul)
...
void setup() {
  ...
  //delay(500); //debug
  spark_io.sp_get_hardware_preset_number();   // Try to use get_hardware_preset_number() to pre-load the correct number
  spark_io.sp_get_preset_details(0x7F);       // Get the preset details for 0-3
  spark_io.sp_get_preset_details(0x0100);     // Get the current preset details     
...
void loop() {
  ...
  // Device connected
  spark_io.sp_process();           // Process commands from Spark
  spark_io.app_process();

  // Messages from the amp
  if (spark_io.sp_get_message(&cmdsub, &msg, &preset)) { 
    ...
    if (cmdsub == 0x0338) {
      ...
      spark_io.sp_get_preset_details(0x0100);
    }      
  ...
  if ((sw_val[0] == HIGH)&&(!isPedalMode)) {  
    pre = 0;
    spark_io.sp_change_hardware_preset(pre);
    spark_io.sp_get_preset_details(0x0100);
    spark_io.app_change_hardware_preset(pre);         // Relay the same change to the app
  }
  else if ((sw_val[1] == HIGH)&&(!isPedalMode)) {  
    pre = 1;
    spark_io.sp_change_hardware_preset(pre);
    spark_io.sp_get_preset_details(0x0100);
    spark_io.app_change_hardware_preset(pre); 
  }
  ...
  // Drive
    ...
    if (presets[5].effects[2].OnOff == true) {
      spark_io.sp_turn_effect_onoff(presets[5].effects[2].EffectName,false);
      spark_io.app_turn_effect_onoff(presets[5].effects[2].EffectName,false);
      presets[5].effects[2].OnOff = false;
    }
    else {
      spark_io.sp_turn_effect_onoff(presets[5].effects[2].EffectName,true);
      spark_io.app_turn_effect_onoff(presets[5].effects[2].EffectName,true);
  ...
  // Modulation
    ...
    if (presets[5].effects[4].OnOff == true) {
      spark_io.sp_turn_effect_onoff(presets[5].effects[4].EffectName,false);
      spark_io.app_turn_effect_onoff(presets[5].effects[4].EffectName,false);
      presets[5].effects[4].OnOff = false;
    }
    else {
      spark_io.sp_turn_effect_onoff(presets[5].effects[4].EffectName,true);
      spark_io.app_turn_effect_onoff(presets[5].effects[4].EffectName,true);
  ...
  // Delay
    ...
    if (presets[5].effects[5].OnOff == true) {
      spark_io.sp_turn_effect_onoff(presets[5].effects[5].EffectName,false);
      spark_io.app_turn_effect_onoff(presets[5].effects[5].EffectName,false);
      presets[5].effects[5].OnOff = false;
    }
    else {
      spark_io.sp_turn_effect_onoff(presets[5].effects[5].EffectName,true);
      spark_io.app_turn_effect_onoff(presets[5].effects[5].EffectName,true);
  ...
  // Reverb
    ...
    if (presets[5].effects[6].OnOff == true) {
      spark_io.sp_turn_effect_onoff(presets[5].effects[6].EffectName,false);
      spark_io.app_turn_effect_onoff(presets[5].effects[6].EffectName,false);
      presets[5].effects[6].OnOff = false;
    }
    else {
      spark_io.sp_turn_effect_onoff(presets[5].effects[6].EffectName,true);
      spark_io.app_turn_effect_onoff(presets[5].effects[6].EffectName,true);
